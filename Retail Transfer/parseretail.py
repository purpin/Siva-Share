...
class ParseRetail:
    ...
    def __init__(self, csv_file):
        ...
        self.list_sku = list_sku.values.tolist()        #list of lists - each sub-list is the description(name) of items in one invoice
        
        #Calculate total value of each invoice
        sum_value = self.DF.groupby(['InvoiceNo']).sum()['Value']
        
        #Create a dataframe containing all the fulfilled invoices
        self.fulfilled_DF = self.DF[self.DF['Value'] > 0]

        #Create a dataframe containing the values of all fulfilled invoices
        self.sum_value_fulfilled = sum_value[sum_value > 0]

        #Calculate support values for all invoices, invoices with one item, and invoices with multiple items
        ...
        #Calculate confidence and lift values 
...
#Function to calculate support for invoices containing n, <n or >n number of items
